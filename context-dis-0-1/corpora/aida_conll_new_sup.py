...
def get_valid_mention_entity_pairs(id_start, id_end, gen_all, dataset_type):
    ...
    for pair in mention_entity_pairs_db:
        ...
        if not Dictionaries.redis_db_obj.\
                missed_g_ent(pair[0], Dictionaries.dataset_source,
        ...
        candidate_with_prior_db = Dictionaries.prior_db_conn_obj.\
            fetch_entity_by_mention_size(normalize_form(pair[1]), 50)
        ...
        for negative_entity in negative_entity_list:
            log.info("Negative pairs generation %s, %s, %s\r" % (str(pair[0]), pair[1],
                                                                 negative_entity))
