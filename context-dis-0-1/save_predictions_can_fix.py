...
def save_model_predictions_extended(model, data_source, data_type, d_test, res_features, docs_dict, top_k=1):
    ...
    for q_id_group_result in all_group_results:
        doc_id_q_ent_lists_dict[docs_dict[q_id_group_result[0]]].append(q_id_group_result)
...
def main(data_source, data_type, iteration, model_suffix='', feature_suffix='', save_format='default', top_k=3):
    ...
    d_ctx_raw = get_feature_results('basic_ctx_can_fixed', data_type, data_source)
    d_ctx = fetch_all_features('basic_ctx_can_fixed', data_type, data_source)
    d_coref = fetch_all_features('basic_coref_can_fixed', data_type, data_source)
    ...
    if iteration == 0:
        model_name = '4900_6_ctx_coref_can_fixed'  # local model
