...
def get_top_k_feature_vector(case, id_ents_list, top_k=3, mention_top_k=3) ->list:
    qid = case[0]
    ...
    for coh_qid, candidates in id_ents_list:
        ...
        if candidate_coh_features:
            max_candidate_coh_features.append(np.max(np.array(candidate_coh_features), axis=0))

    if max_candidate_coh_features:
        top_k_features = []
        max_candidate_coh_features = np.vstack(max_candidate_coh_features)
        max_fea_shape = max_candidate_coh_features.shape
        for col in range(max_fea_shape[1]):
            col_data = max_candidate_coh_features[:, col]
            col_top_k = sorted(col_data, reverse=True)[:mention_top_k]
            if max_fea_shape[0] < mention_top_k:  # append empty features
                col_top_k.extend([0] * (mention_top_k - max_fea_shape[0]))
            assert len(col_top_k) == mention_top_k
            top_k_features.extend(col_top_k)
        return top_k_features
    return [0] * ((9 + 2) * mention_top_k)  # empty feature, 9 link-related, 2 string-related
