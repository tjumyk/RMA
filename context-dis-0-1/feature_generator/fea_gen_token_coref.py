...
def process(cases):
    ...
    if cases:
        ...
        mention = cases[0][3]
        doc_ents_dict_stfd = const_vars_dict.redis_db_obj.get_ents_stfd_by_doc(doc_id)
        ...
        candidate_with_prior_db_stfd = const_vars_dict.prior_db_conn_obj.\
            fetch_entity_by_mention_size(normalize_form(coref_men_stfd), 0)
        ...
        candidate_with_prior_db_conll = const_vars_dict.prior_db_conn_obj. \
            fetch_entity_by_mention_size(normalize_form(coref_men_conll), 0)
    ...
    for case in cases:
        ...
        if not const_vars_dict.redis_db_obj.is_processed_items_redis(
            ...
            fea_vec = [fea_id] + get_feature_vector(case, ent_prior_dict_stfd,
                                                    ent_prior_dict_conll)
...
def get_feature_vector(case, ent_prior_dict_stfd, ent_prior_dict_conll):
    """
    ...
    prior = float(case[1])
    max_prior_entity = const_vars_dict.max_ent_priors_dict[doc_id].get(entity, prior)
    ...
    feature_vec = []
    feature_vec += [max_prior_entity, prior_stfd_ner_coref, prior_conll_coref] +\
