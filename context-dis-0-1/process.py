...
conn_psql_kongzi2 = psycopg2.connect(database='aida', user='zding', password='dingzishuo', host='mozi',
                                     port=5432)
conn_psql = psycopg2.connect(database='zding', user='zding', password='dingzishuo', host='mozi',
                             port=5432)


def get_feature_results(exp_id, d_t, data_source='aida_conll'):
    print('Getting feature %s from %s %s' % (exp_id, data_source, d_t))
...
def fetch_all_features(exp_id, data_type, data_source='aida_conll'):
    ...
    return raw_to_features(res_feas)


def raw_to_features(res_feas):
...
def evalerror_detail_log(preds, dt, d_tal_size):
    ...
    recall = float(len(matched_ids)) / d_tal_size
    f1 = 2 * precision * recall / (precision + recall)
    return len(matched_ids), precision, recall, f1, matched_ids, duplicates_results, wrong_results
...
def save_model(model, name, model_dir_path='./new_models_14_Aug'):
...
def load_model(name, model_dir_path='./new_models_14_Aug'):
    ...
    for x in n_estimators:
        ...
        for dep in max_depths:
            ...
            if train_mode:
                ...
                param = {'max_depth': dep, 'eta': 0.01, 'silent': 1, 'objective': 'rank:pairwise',
            ...
            print(
                "n_estimators: {}, max_depth: {}, acc_validation: {}, corr_num: {}".format(num_round, dep, a[2], a[0]))
...
def save_local_model_predictions(model, data_source, data_type, d_test, raw_test, docs_dict, top_k=None,
    ...
    for key, vals in doc_id_q_ent_lists_dict.items():
        conn.hset('doc-predicted-ents:%s:%s:%s' % (data_source, data_type, redis_suffix), key, json.dumps(vals))

    if top_k is not None:
        conn.hset('doc-predicted-ents-top-k:%s:%s:%s' % (data_source, data_type, redis_suffix), 'test', repr(top_k_indices_test))
...
def save_model_predictions_extended(model, data_source, data_type, d_test, res_features, docs_dict, top_k=1,
    ...
    for q_id_group_result in all_group_results:
        ...
        conn.hset('mention-candidate-filter:%s:%s:%s' % (data_source, data_type, redis_suffix), q_id, json.dumps(group_results))

    for key, vals in doc_id_q_ent_lists_dict.items():
        conn.hset('doc-predicted-ents-ext:%s:%s:%s' % (data_source, data_type, redis_suffix), key, json.dumps(vals))


def save_predictions_main(data_source, data_type, iteration, model_suffix='', feature_suffix='', save_format='default',
                          top_k=3):
...
def filter_out_tf_idf_entropy(features):
    print('Filtering out tf-idf-entropy features')
    # tf-id-entropy is at [9:20] in each vector
    fea_indices = list(range(0, 9)) + list(range(20, features.shape[-1]))
    return features[:, fea_indices]


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Utility for processing data and models in CLI')
    ...
    parser.add_argument('-p', '--prefix', help='Prefix for feature names and model paths', default='default')
    parser.add_argument('-l', '--local_prefix', help='Prefix for local feature names and model paths. If not provided, '
                                                     'use the value of prefix', default='')
    parser.add_argument('-L', '--local_model_name', help='Override the local model to use', default='')
    parser.add_argument('-i', '--iterations', type=int, help='Number of iterations for training', default=4900)
    parser.add_argument('-d', '--max_depth', type=int, help='Max depth for training', default=6)
    parser.add_argument('-g', '--global_iteration', type=int, help='Number of iteration of the global model', default=0)
    parser.add_argument('-s', '--save_prediction_format', help='Format of the saved predictions', default='default')
    parser.add_argument('-t', '--top_k', type=int, help='Top-K predictions to save', default=3)
    parser.add_argument('-c', '--coherence_type', help='Type of coherence feature', default='coh')
    parser.add_argument('--no_tf_idf_entropy', help='Hide tf, idf and entropy features in local features for ablation '
                                                    'test', action='store_true')
    parser.add_argument('-o', '--output_file', help='Output TSV file for detailed results')
    ...
    prefix = args.prefix
    local_prefix = args.local_prefix or prefix
    local_model_name = args.local_model_name
    iterations = args.iterations
    max_depth = args.max_depth
    global_iteration = args.global_iteration
    save_prediction_format = args.save_prediction_format
    top_k = args.top_k
    coherence_type = args.coherence_type
    no_tf_idf_entropy = args.no_tf_idf_entropy
    output_file = args.output_file

    print('=== Action: %s, Data Source: %s, Data Type: %s, Prefix: %s' % (action, data_source, data_type, prefix))

    if action == 'train_local':
        print('--- Iterations: %d, Max Depth: %d' % (iterations, max_depth))

        log('Loading features')
        fea_ctx = fetch_all_features('%s:fea_ctx' % prefix, data_type, data_source)
        if no_tf_idf_entropy:
            fea_ctx = filter_out_tf_idf_entropy(fea_ctx)
        fea_coref = fetch_all_features('%s:fea_coref' % prefix, data_type, data_source)
        fea_local = combine_features(fea_ctx, fea_coref)
        total_mentions = get_total_mentions(data_source, data_type)
        fea_local_xgboost = trans_data(fea_local)

        log('Start training')
        param = {'max_depth': max_depth, 'eta': 0.01, 'silent': 1, 'objective': 'rank:pairwise',
                 'min_child_weight': 0.01}
        bst = xgb.train(param, fea_local_xgboost, iterations)
        log('Training finished')

        save_model(bst, 'local_%d_%d' % (iterations, max_depth), 'models_%s' % prefix)

        log('Start evaluation')
        predictions = bst.predict(fea_local_xgboost)
        result = evalerror_detail_log(predictions, fea_local_xgboost, total_mentions)
        log("precision:{},  recall: {}, correct count: {}".format(result[1], result[2], result[0]))
        ...
        log('Loading model')
        bst = load_model('local_%d_%d' % (iterations, max_depth), 'models_%s' % prefix)

        log('Start evaluation')
        predictions = bst.predict(fea_local_xgboost)
        result = evalerror_detail_log(predictions, fea_local_xgboost, total_mentions)
        log("precision:{},  recall: {}, correct count: {}".format(result[1], result[2], result[0]))
    elif action == 'save_predictions':
        print('--- Global Iteration: %d, Coherence Type: %s, Save Prediction Format: %s, Top-K: %d' %
              (global_iteration, coherence_type, save_prediction_format, top_k))

        log('Loading features')
        fea_ctx_raw = get_feature_results('%s:fea_ctx' % local_prefix, data_type, data_source)
        fea_ctx = raw_to_features(fea_ctx_raw)
        if no_tf_idf_entropy:
            # note: fea_ctx_raw does not need to do slicing as we will only use its first column
            fea_ctx = filter_out_tf_idf_entropy(fea_ctx)
        fea_coref = fetch_all_features('%s:fea_coref' % local_prefix, data_type, data_source)
        fea_local = combine_features(fea_ctx, fea_coref)
        docs_dict = fetch_q_ids_docs(data_source)
        ...
        if global_iteration == 0:
            ...
            fea_coh = fetch_all_features('%s:fea_%s%d' % (prefix, coherence_type, global_iteration), data_type, data_source)
            fea_all = combine_features(fea_local, fea_coh)

        log('Loading model')
        if global_iteration == 0:
            bst = load_model(local_model_name or 'local_%d_%d' % (iterations, max_depth), 'models_%s' % local_prefix)
        else:
            bst = load_model('global_%s%d_%d_%d' % (coherence_type, global_iteration, iterations, max_depth), 'models_%s' % prefix)

        log('Saving predictions')
        suffix = socket.gethostname()  # use hostname as an easy way to distinguish data generated by different hosts
        if save_prediction_format == 'default':
            save_local_model_predictions(bst, data_source, data_type, fea_all, fea_ctx_raw, docs_dict,
                                         redis_suffix=suffix)
        elif save_prediction_format == 'ext':
            save_model_predictions_extended(bst, data_source, data_type, fea_all, fea_ctx_raw, docs_dict, top_k,
                                            redis_suffix=suffix)
        else:
            raise ValueError('invalid save format: %s' % save_prediction_format)
    elif action == 'train_global':
        print('--- Global Iteration: %d, Coherence Type: %s, Iterations: %d, Max Depth: %d' %
              (global_iteration, coherence_type, iterations, max_depth))

        log('Loading features')
        fea_ctx = fetch_all_features('%s:fea_ctx' % local_prefix, data_type, data_source)
        if no_tf_idf_entropy:
            fea_ctx = filter_out_tf_idf_entropy(fea_ctx)
        fea_coref = fetch_all_features('%s:fea_coref' % local_prefix, data_type, data_source)
        fea_local = combine_features(fea_ctx, fea_coref)
        ...
        if global_iteration == 0:
            raise ValueError('Global iteration must be at least 1 for training')

        fea_coh = fetch_all_features('%s:fea_%s%d' % (prefix, coherence_type, global_iteration), data_type, data_source)
        fea_all = combine_features(fea_local, fea_coh)
        fea_all_xgboost = trans_data(fea_all)

        log('Start training')
        param = {'max_depth': max_depth, 'eta': 0.01, 'silent': 1, 'objective': 'rank:pairwise',
                 'min_child_weight': 0.01}
        bst = xgb.train(param, fea_all_xgboost, iterations)
        log('Training finished')

        save_model(bst, 'global_%s%d_%d_%d' % (coherence_type, global_iteration, iterations, max_depth),
                   'models_%s' % prefix)

        log('Start evaluation')
        predictions = bst.predict(fea_all_xgboost)
        result = evalerror_detail_log(predictions, fea_all_xgboost, total_mentions)
        log("precision:{},  recall: {}, correct count: {}".format(result[1], result[2], result[0]))
    elif action == 'test_global':
        print('--- Global Iteration: %d, Coherence Type: %s, Iterations: %d, Max Depth: %d' %
              (global_iteration, coherence_type, iterations, max_depth))
        ...
        if global_iteration == 0:
            raise ValueError('Global iteration must be at least 1 for testing')
        ...
        log('Loading model')
        bst = load_model('global_%s%d_%d_%d' % (coherence_type, global_iteration, iterations, max_depth),
                         'models_%s' % prefix)
        ...
        if output_file:
            with open(output_file, 'w') as f_out:
                match_indices = result[4]
                for mid in fea_ctx[match_indices, 0]:
                    f_out.write('%d\n' % int(mid))
    else:
