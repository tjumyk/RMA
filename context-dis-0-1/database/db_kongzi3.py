...
class PostgreSQL(object):
    ...
    def fetch_training_mentions_kore(self):
        cur = self.conn.cursor()
        sql = 'select surfaceform, annotation from kore'
        cur.execute(sql)
        rows = cur.fetchall()
        cur.close()
        return rows
    ...
    def fetch_mention_entity_pairs(self, dataset_source, dataset_type):
        cur = self.conn.cursor()
        sql = "SELECT id, surfaceform, annotation, doc_text, doc_id FROM %s WHERE type=\'%s\' AND annotation != 'none'"
        cur.execute(sql % (dataset_source, dataset_type))
        rows = cur.fetchall()
        cur.close()
        return rows

    def fetch_mention_entity_pairs_conll_testb(self, dataset_type):
        cur = self.conn.cursor()
        sql = "SELECT aida_conll.id, aida_conll.surfaceform, aida_conll.annotation, aida_conll.doc_text, aida_conll.doc_id FROM aida_conll WHERE aida_conll.type=\'%s\' AND annotation != 'NIL'"
        cur.execute(sql % dataset_type)
        rows = cur.fetchall()
        cur.close()
        return rows

    def fetch_mention_entity_pairs_by_ids(self, dataset_source, dataset_type, id_start, id_end):
        cur = self.conn.cursor()
        sql = "SELECT id, surfaceform, annotation, doc_text, begoffset, doc_id FROM %s WHERE type=\'%s\' AND annotation != 'none' AND annotation !='NIL' AND id BETWEEN %s and %s;"
        cur.execute(sql % (dataset_source, dataset_type, id_start, id_end))
        rows = cur.fetchall()
        cur.close()
        return rows

    def fetch_mention_entity_pairs_conll_testb_by_size(self, id_start, id_end, dataset_type):
        cur = self.conn.cursor()
        sql = "SELECT aida_conll.id, aida_conll.surfaceform, aida_conll.annotation, aida_conll.doc_text, aida_conll.doc_id FROM aida_conll WHERE aida_conll.type=\'%s\' AND annotation != 'NIL' AND aida_conll.id BETWEEN %s and %s;"
        cur.execute(sql % (dataset_type, id_start, id_end))
        rows = cur.fetchall()
        cur.close()
        return rows
    ...
    def fetch_mention_entity_pairs_kore(self)-> List[tuple]:
        ...
        '''
        cur = self.conn.cursor()
        sql = "SELECT kore.id, kore.surfaceform, kore.annotation, kore.doc_text FROM kore WHERE kore.annotation != 'NIL'"
        cur.execute(sql)
        rows = cur.fetchall()
        cur.close()
        return rows
    ...
    def fetch_all_entities_context_from_training(self):
        cur = self.conn.cursor()
        sql = "SELECT entity_id, content FROM wiki_article_training"
        cur.execute(sql)
        row = cur.fetchall()
        cur.close()
        # print(row)
    ...
    def save_coref_doc(self, coreference=None):
        cur = self.conn.cursor()
        cur.execute(
            "INSERT INTO doc_coref (start_position, end_position, start_index, end_index, is_representative_mention,"
            "sentence_number, doc_id, word_id, coref_id, data_source, doc_id_hash) VALUES (%s, %s, %s, %s, %s, %s, %s,"
            "%s, %s, %s, %s)",
            (coreference['characterOffsetBegin'], coreference['characterOffsetEnd'], coreference['startIndex'],
             coreference['endIndex'], coreference['isRepresentativeMention'], coreference['sentNum'] - 1,
             coreference['doc_id'], coreference['text'], coreference['coref_id'], coreference['data_source'],
             coreference['doc_id_hash']))
        self.conn.commit()
        cur.close()
    ...
    def fetch_id_men_doc_source(self, dataset_source):
        cur = self.conn.cursor()
        sql = "select id, surfaceform, doc_id from %s;"
        cur.execute(sql % dataset_source)
        rows = cur.fetchall()
        cur.close()
        return rows

    def fetch_id_men_source(self, dataset_source):
        cur = self.conn.cursor()
        sql = "select id, surfaceform from %s;"
        cur.execute(sql % dataset_source)
        rows = cur.fetchall()
        cur.close()
        return rows

    def fetch_doc_mens_beg_pos_source(self, dataset_source):
        cur = self.conn.cursor()
        sql = "select doc_id, surfaceform, begoffset from %s where annotation !='NIL' and annotation != 'none';"
        cur.execute(sql % dataset_source)
        rows = cur.fetchall()
        cur.close()
        return rows