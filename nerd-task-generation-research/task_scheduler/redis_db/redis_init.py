import task_scheduler.redis_db
import json
import redis
import os
...
class RedisConn(object):
    def __init__(self):
        config_path = task_scheduler.redis_db.__path__[0]
    ...
    def get_men_tokens_redis(self, data_source, doc_id):
        if data_source == 'aida_conll':
            data_source = 'conll'
        tokens = self.conn.hmget('men-tok-ner-spacy-' + data_source, doc_id)[0]
        return json.loads(tokens) if tokens else {}

    def get_men_ents_tokens_idf_redis(self, men):
        ent_tok_dict = self.conn.hmget('idf:::50', men)[0]
        return json.loads(ent_tok_dict) if ent_tok_dict else {}

    def get_men_ents_tokens_entropy_redis(self, men):
        ent_tok_dict = self.conn.hmget('entropy:::50', men)[0]
        return json.loads(ent_tok_dict) if ent_tok_dict else {}

    def get_men_ents_tokens_tf_redis(self, men):
        ent_tok_dict = self.conn.hmget('tf:::50', men)[0]
        return json.loads(ent_tok_dict) if ent_tok_dict else {}

    def save_men_ents_tokens_idf_redis(self, men, ent_idf_dict):
        return self.conn.hset('idf:::50', men, json.dumps(ent_idf_dict))

    def save_men_ents_tokens_entropy_redis(self, men, ent_entropy_dict):
        return self.conn.hset('entropy:::50', men, json.dumps(ent_entropy_dict))

    def save_men_ents_tokens_tf_redis(self, men, ent_tf_dict):
        return self.conn.hset('tf:::50', men, json.dumps(ent_tf_dict))
    ...
    def get_id_ents_by_doc_redis(self, doc_id):
        id_ents = self.conn.hmget('doc-predicted-ents-coref-new', doc_id)[0]
    ...
    def get_men_relaxed_form_redis(self, men):
        ...
        return relaxed_men if relaxed_men else None
